...
const AssembleShip = ({ accountAddress, asteroid, crew, lot, onSetAction, _disabled }) => {
  ...
  const disabledReason = useMemo(() => {
    ...
    if (assemblyStatus === 'READY') {
      return getCrewDisabledReason({
        accountAddress, asteroid, crew, permission: Permission.IDS.ASSEMBLE_SHIP, permissionTarget: lot?.building
      });
    } else if (!currentAssembly?._isMyAction) {
      return 'in use';
    }
  }, [_disabled, accountAddress, assemblyStatus, asteroid, crew, currentAssembly, lot?.building]);
