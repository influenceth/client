...
const getApproveEthCall = ({ amount }) => ({
  ...
  entrypoint: Systems.ApproveEth.entrypoint,
  calldata: Systems.ApproveEth.getCalldata({ amount }),
});

const getRunSystemCall = (system, input) => ({
  contractAddress: process.env.REACT_APP_STARKNET_DISPATCHER,
  entrypoint: Systems[system].entrypoint,
  calldata: Systems[system].getCalldata(input),
});
...
// const configDefault = {
//   abi: configs.Dispatcher,
//   getCalls: (name, inputs) => ([
//     getRunSystemCall(name, inputs)
//   ]),
