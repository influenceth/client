...
const useNameAvailability = (configType) => {
  const createAlert = useStore(s => s.dispatchAlertLogged);

  const validateName = useCallback((name, suppressAlert) => {
    let err = '';

    const standardError = Name.getNameError(name, Name.getType(configType));
  ...
  const getNameAvailability = useCallback(async (name, suppressAlert) => {
    try {
      ...
      const nameCollisions = await api.getNameUse(configType, name);
